...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [revenue, topItems, paymentMethods, tableRevenue, hourlyBreakdown] =
        await Promise.all([
          // Revenue breakdown
          pool.query(
            `
...
router.get("/menu/categories", async (req, res) => {
  try {
    ...
    logger.error("[ERR] List categories:", err);
    res.status(500).json({ error: "Kategoriler yüklenemedi" });
  }
});

router.post("/menu/categories", requireRole("owner"), async (req, res) => {
  const { name } = req.body;
  if (!name || !name.trim()) {
  ...
  try {
    ...
    logger.error("[ERR] Add category:", err);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (parseInt(inUse.rows[0].count) > 0) {
        return res
          .status(409)
          .json({ error: "Bu kategoride ürün var, önce ürünleri taşıyın" });
      }
      await pool.query("DELETE FROM categories WHERE id = $1", [id]);
      res.json({ message: "Kategori silindi" });
    } catch (err) {
      logger.error("[ERR] Delete category:", err);
...
router.post("/menu", requireRole("owner"), async (req, res) => {
  ...
  if (!name || !category || price === undefined) {
    return res
      .status(400)
      .json({ error: "Name, category, and price required" });
  }
...
router.delete("/tables/:tableId", requireRole("owner"), async (req, res) => {
  ...
  try {
    ...
    if (parseInt(activeSessions.rows[0].count) > 0) {
      return res
        .status(400)
        .json({ error: "Aktif oturumu olan masa silinemez" });
    }
  ...
  async (req, res) => {
    ...
    try {
      ...
      const balanceResult = await pool.query(
        "SELECT get_remaining_balance($1)",
        [sessionId],
      );
