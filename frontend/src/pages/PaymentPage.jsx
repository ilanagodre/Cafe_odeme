...
export default function PaymentPage() {
  ...
  const mySplit = splitResult?.splits?.find(s => s.participantId === participantId);
  const myIndividualSplit = individualSplit?.splits?.find(s => s.participantId === participantId);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (paymentMode === 'all') {
        ...
        const targetSplit = individualSplit?.splits?.find(s => s.participantId === selectedForId);
      ...
      if (res.ok) {
        ...
        if (data.allSettled || newBalance <= 0) {
          setTimeout(() => navigate(`/table/${sessionToken}/${participantId}`), 2500);
        }
      } else {
  ...
  const handleThreeDSSuccess = () => {
    ...
    const newBalance = remainingBalance - (paymentMode === 'all' ? remainingBalance : myIndividualSplit?.amount || 0);
    if (newBalance <= 0) {
      setTimeout(() => navigate(`/table/${sessionToken}/${participantId}`), 2500);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-sm">
          ...
          <h1 className="text-2xl font-bold text-green-700 mb-2">Hesap Kapandı!</h1>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-gradient-to-r from-red-500 to-orange-500 rounded-xl p-5 text-white shadow-lg">
          ...
          <p data-testid="remaining-balance-header" className="text-4xl font-bold mt-1">{remainingBalance.toFixed(2)}₺</p>
        </div>
        ...
        {paymentMethod === 'iyzico' && showCardForm && (
          <div className="bg-white rounded-xl p-4 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-800 mb-4">Kart Bilgilerini Gir</h3>
            {iyzicoError && (
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="space-y-2">
              {individualSplit.splits.map(split => {
                const participant = sessionState.participants.find(p => p.id === split.participantId);
                const isMe = split.participantId === participantId;
                const hasPaid = sessionState.payments?.some(p => p.participant_id === split.participantId && p.status === 'completed');
                const isTargetSelected = selectedForId === split.participantId;

                return (
                  <div
                    key={split.participantId}
                    onClick={() => paymentMode === 'other' && !hasPaid && setSelectedForId(split.participantId)}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold shadow" style={{ backgroundColor: participant?.color_code }}>
                        {participant?.name[0]}
                      </div>
                      <div>
                        <p className="font-medium text-sm text-gray-800">
                          {participant?.name} {isMe && <span className="text-indigo-600">(sen)</span>}
                        </p>
                        {hasPaid && <p className="text-xs text-green-600">✓ Ödedi</p>}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className={`font-bold text-lg ${hasPaid ? 'text-green-600 line-through' : 'text-gray-800'}`}>
        ...
        {/* Payment Mode Selection (only for test payment) */}
        {paymentMethod === 'test' && (
        <div className="bg-white rounded-xl p-4 shadow-sm space-y-3">
          ...
          <button
            ...
            onClick={() => { setPaymentMode('self'); setSelectedForId(null); }}
            className={`w-full p-4 rounded-xl border-2 flex items-center justify-between transition-all ${
              paymentMode === 'self' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-100 hover:border-gray-200'
            }`}
          >
            <div className="flex items-center gap-3">
              <span className="text-2xl">💳</span>
              <div className="text-left">
                <p className="font-semibold text-gray-800">Kendi Borcumu Öde</p>
                <p className="text-sm text-gray-500">{myIndividualSplit?.amount?.toFixed(2)}₺</p>
              </div>
            </div>
            {paymentMode === 'self' && <div className="w-5 h-5 bg-indigo-500 rounded-full" />}
          </button>
          ...
          {remainingBalance > (mySplit?.amount || 0) && (
            <button
              ...
              onClick={() => { setPaymentMode('all'); setSelectedForId(null); setSelectedOrderIds([]); }}
              className={`w-full p-4 rounded-xl border-2 flex items-center justify-between transition-all ${
                paymentMode === 'all' ? 'border-amber-500 bg-amber-50' : 'border-gray-100 hover:border-gray-200'
              }`}
            >
              <div className="flex items-center gap-3">
                <span className="text-2xl">🎉</span>
                <div className="text-left">
                  ...
                  <p className="text-sm text-gray-500">Herkesin borcu ({remainingBalance.toFixed(2)}₺)</p>
                </div>
              </div>
              {paymentMode === 'all' && <div className="w-5 h-5 bg-amber-500 rounded-full" />}
            </button>
          )}
          ...
          <button
            ...
            onClick={() => { setPaymentMode('item'); setSelectedForId(null); }}
            className={`w-full p-4 rounded-xl border-2 flex items-center justify-between transition-all ${
              paymentMode === 'item' ? 'border-pink-500 bg-pink-50' : 'border-gray-100 hover:border-gray-200'
            }`}
          >
            <div className="flex items-center gap-3">
              <span className="text-2xl">🎁</span>
              <div className="text-left">
                ...
                <p className="text-sm text-gray-500">Birkaç sipariş seç, öde</p>
              </div>
            </div>
            {paymentMode === 'item' && <div className="w-5 h-5 bg-pink-500 rounded-full" />}
          </button>

          {/* Other */}
          {splitResult?.splits?.filter(s => {
            const p = sessionState.participants.find(x => x.id === s.participantId);
            return s.participantId !== participantId && !sessionState.payments?.some(pay => pay.participant_id === s.participantId && pay.status === 'completed');
          }).length > 0 && (
            <div>
              <button
                ...
                className={`w-full p-4 rounded-xl border-2 flex items-center justify-between transition-all ${
                  paymentMode === 'other' ? 'border-green-500 bg-green-50' : 'border-gray-100 hover:border-gray-200'
                }`}
              >
                <div className="flex items-center gap-3">
                  <span className="text-2xl">💚</span>
                  <div className="text-left">
                    <p className="font-semibold text-gray-800">Birinin Borcunu Öde</p>
                    <p className="text-sm text-gray-500">Aşağıdan seç</p>
                  </div>
                </div>
                {paymentMode === 'other' && <div className="w-5 h-5 bg-green-500 rounded-full" />}
              </button>
            </div>
          )}
          ...
          {paymentMode === 'item' && (
            <div className="mt-3 bg-pink-50 rounded-xl p-4 border border-pink-200">
              <p className="text-sm font-semibold text-pink-700 mb-3">🎁 Ödemek istediğin siparişleri seç:</p>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {sessionState.orders
                  ?.filter(o => o.status !== 'cancelled' && !o.paid_by)
                  .map(order => {
                    const orderedBy = sessionState.participants.find(p => p.id === order.ordered_by);
                    const isSelected = selectedOrderIds.includes(order.id);
                    return (
                      <label
                        key={order.id}
                        className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${
                          isSelected ? 'bg-pink-100 border-2 border-pink-400' : 'bg-white border border-gray-200'
                        }`}
                      >
                        <input
                          type="checkbox"
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-sm text-gray-800">{order.name} × {order.quantity}</p>
                          <p className="text-xs text-gray-500">
                            {orderedBy?.name || '—'} sipariş verdi
                          </p>
                        </div>
                        <p className="font-bold text-sm text-gray-800">{parseFloat(order.total_price).toFixed(2)}₺</p>
                      </label>
                    );
                  })}
              </div>
              ...
              {selectedOrderIds.length > 0 && (
                <div className="mt-3 pt-3 border-t border-pink-200 flex justify-between items-center">
                  <p className="text-sm text-pink-600">{selectedOrderIds.length} sipariş seçildi</p>
        ...
        {paymentMethod === 'test' && paymentError && (
          <div data-testid="payment-error" className="bg-red-50 text-red-600 p-4 rounded-xl text-center text-sm">
            ⚠️ {paymentError}
          </div>
        )}
                  ...
                  {paymentMode === 'all'
                    ...
                    : paymentMode === 'other' && selectedForId
                      ...
                      : paymentMode === 'item' && selectedOrderIds.length > 0
                        ? `${selectedOrderIds.length} Sipariş Öde (${sessionState.orders?.filter(o => selectedOrderIds.includes(o.id)).reduce((s, o) => s + parseFloat(o.total_price), 0).toFixed(2)}₺)`
        ...
        {/* Success */}
        {paymentDone && (
          <div data-testid="payment-success" className="bg-green-100 rounded-xl p-6 text-center">
            <p className="text-4xl mb-2">✅</p>
            <p className="text-lg font-bold text-green-800">{paymentMsg}</p>
            <p className="text-sm text-green-600 mt-1">
              {sessionState?.remainingBalance <= 0 ? 'Hesap kapandı! Afiyet olsun 🎉' : 'Diğerlerinin ödemesi bekleniyor...'}
            </p>
          </div>
        )}
