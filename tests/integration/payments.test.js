...
const { SESSION_ID, SESSION_TOKEN, PARTICIPANT_ID_1, PARTICIPANT_ID_2, ORDER_ID_1, mockSession, mockParticipant2 } = require('../helpers/fixtures');
...
describe('Payment Endpoints', () => {
  ...
  describe('POST /api/payment', () => {
    ...
    test('should process payment successfully', async () => {
      ...
      mockPool.query
        ...
        .mockResolvedValueOnce({ rows: [{ id: 'payment-1', ...payload, status: 'completed' }] }) // Insert payment
  ...
  describe('POST /api/payment/full', () => {
    ...
    test('should not close session if balance remains', async () => {
      ...
      mockPool.query
        ...
        .mockResolvedValueOnce({ rows: [{ get_remaining_balance: '200' }] }); // Get new remaining balance (still positive)
    ...
    test('should return already paid message for zero balance', async () => {
      ...
      mockPool.query
        .mockResolvedValueOnce({ rows: [{ id: SESSION_ID }] }) // Find active session
        .mockResolvedValueOnce({ rows: [{ get_remaining_balance: '0' }] }); // Get remaining balance
  ...
  describe('POST /api/payment/for', () => {
    ...
    test('should pay for another person successfully', async () => {
      ...
      mockPool.query
        ...
        .mockResolvedValueOnce({ rows: [{ id: 'payment-1', ...payload }] }) // Insert payment
        .mockResolvedValueOnce({ rows: [] }) // Update paid_amount
        .mockResolvedValueOnce({ rows: [{ get_remaining_balance: '200' }] }); // Get remaining balance

      const res = await request(app)
        .post('/api/payment/for')
        .send(payload);
    ...
    test('should return 404 for non-existent target participant', async () => {
      const payload = {
        sessionToken: SESSION_TOKEN,
        paidBy: PARTICIPANT_ID_1,
        targetParticipantId: 'INVALID-ID',
        amount: 100
      };
  ...
  describe('POST /api/payment/item', () => {
    ...
    test('should pay for specific items', async () => {
      ...
      mockPool.query
        .mockResolvedValueOnce({ rows: [{ id: SESSION_ID }] }) // Find active session
        .mockResolvedValueOnce({ rows: [{ id: ORDER_ID_1, total_price: '100', name: 'Kahve' }] }) // Get orders
        .mockResolvedValueOnce({ rows: [{ id: 'payment-1', amount: '100' }] }) // Insert payment
