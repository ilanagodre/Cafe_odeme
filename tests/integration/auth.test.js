...
const { makeToken, TEST_PIN, TEST_PIN_HASH, STAFF_ID } = require('../helpers/fixtures');
...
describe('Authentication Endpoints', () => {
  ...
  describe('POST /api/auth/login', () => {
    ...
    test('should login with correct PIN', async () => {
      ...
      mockPool.query
        .mockResolvedValueOnce({ rows: [{ id: STAFF_ID, name: 'Test User', role: 'owner', pin_hash: TEST_PIN_HASH }] }) // Get active users
    ...
    test('should handle multiple users and find correct one', async () => {
      ...
      mockPool.query
        .mockResolvedValueOnce({
          rows: [
            { id: 'user-1', name: 'Wrong User', role: 'waiter', pin_hash: '$2b$10$wronghash' },
            { id: STAFF_ID, name: 'Correct User', role: 'owner', pin_hash: TEST_PIN_HASH }
          ]
        }) // Get active users
        .mockResolvedValueOnce({ rows: [] }); // Update last_login
  ...
  describe('GET /api/auth/me', () => {
    ...
    test('should return user info with valid token', async () => {
      ...
      mockPool.query
        .mockResolvedValueOnce({ rows: [{ id: STAFF_ID, name: 'Test User', role: 'owner', is_active: true }] }); // Get user
